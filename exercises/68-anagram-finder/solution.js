...
function sortLetters(word) {
  // CORREGIDO: Se debe convertir a minúsculas antes de dividir y ordenar,
  // para que la clave canónica sea independiente de mayúsculas.
  return word
    .toLowerCase()
    .split('')
    .sort()
...
function areAnagrams(wordA, wordB) {
  // Las palabras de distinta longitud nunca pueden ser anagramas
...
function groupAnagrams(words) {
  ...
  // Construir un mapa: clave canónica → palabras del grupo
  ...
  // Extraer todos los grupos del mapa
...
function findAnagramsOf(targetWord, wordList) {
  ...
  return wordList.filter((word) => {
    // Excluir la misma palabra (comparación insensible a mayúsculas)
...
function getLargestAnagramGroup(words) {
  ...
  // Encontrar el grupo con más elementos usando reduce
...
function filterWordsWithMinAnagrams(words, minAnagrams) {
  ...
  // Construir el mapa de grupos una sola vez para evitar O(n²) comparaciones
  const familyMap = words.reduce((map, word) => {
    const key = sortLetters(word);
  ...
  return words.filter((word) => {
    ...
    // El número de anagramas de una palabra es el tamaño del grupo menos ella misma
...
function findWordsWithNoAnagrams(words) {
  ...
  // Construir el mapa de grupos una sola vez
  const familyMap = words.reduce((map, word) => {
    const key = sortLetters(word);
    if (!map.has(key)) map.set(key, []);
    map.get(key).push(word);
  ...
  // Retener solo las palabras cuyo grupo tiene exactamente un miembro
  return words.filter((word) => {
    const key = sortLetters(word);
...
function getAnagramStats(words) {
  ...
  // Tamaño promedio de todos los grupos
