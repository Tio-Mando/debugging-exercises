...
function calculateProration(price, daysUsed, totalDays) {
  ...
  // CORREGIDO: Divide entre totalDays (duración real del ciclo) en lugar de un valor fijo
  return Math.round((price / totalDays) * daysUsed * 100) / 100;
}
...
function calculateCancellationRefund(subscription, currentDate = new Date()) {
  ...
  // CORREGIDO: Usa daysRemaining para calcular lo que se debe devolver,
  // no daysUsed (que sería cobrar de nuevo los días ya consumidos)
...
function applyDiscounts(basePrice, discounts) {
  ...
  // CORREGIDO: Los descuentos se suman (aditivos) y se aplican una sola vez,
  // no se encadenan multiplicativamente sobre el precio resultante anterior
