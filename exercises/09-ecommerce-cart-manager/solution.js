...
function addItem(cart, product, quantity) {
  ...
  if (existingItem) {
    // CORREGIDO: Se valida el stock total antes de incrementar la cantidad
...
function removeItem(cart, productId) {
  // CORREGIDO: Primero buscar el índice para manejar correctamente si no existe
...
function applyCoupon(cart, coupon) {
  ...
  // CORREGIDO: Se convierte la fecha de expiración a Date para comparar correctamente
...
function calculateTotals(cart) {
  ...
  // CORREGIDO: El impuesto se calcula sobre el subtotal ya con descuento aplicado
  const tax = discountedSubtotal * TAX_RATE;

  // CORREGIDO: Redondeo correcto a 2 decimales para evitar problemas de precisión (e.g. 0.1 + 0.2)
